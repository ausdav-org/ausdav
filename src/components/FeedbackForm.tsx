...
export default function FeedbackForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const submit = async () => {
    if (!message.trim()) {
      toast({ title: 'Missing message', description: 'Please enter your feedback' });
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (fnResp.status === 429) {
        toast({ title: 'Rate limited', description: 'Too many submissions, please try later' });
        return;
      }

      // Fallback: direct insert using Supabase client
      const { error } = await db.from('feedback').insert([{ message, type: null, is_read: false }]);
      ...
      toast({ title: 'Submission failed', description: 'Could not send feedback' });
    } finally {
      setSubmitting(false);
    }
  };
